...
import require$$1$5, { BrowserWindow, ipcMain, shell, clipboard, session, globalShortcut, dialog, app, Menu, Tray } from "electron";
...
import require$$1$4, { join as join$1 } from "path";
...
function handleBIEvent(event, input) {
  ...
  if (code.indexOf("Key") !== -1)
    code = code.substring(code.indexOf("Key") + 3);
...
function togglePriceCheck(clip = null) {
...
const _MyClipBoard = class _MyClipBoard {
  static delayRestoreClipboard(callback) {
...
__publicField(_MyClipBoard, "clipSave", null);
__publicField(_MyClipBoard, "debounceFun", debounce(() => {
  if (_MyClipBoard.clipSave !== null)
    clipboard.writeText(_MyClipBoard.clipSave);
  _MyClipBoard.clipSave = null;
}, 250));
let MyClipBoard = _MyClipBoard;
...
function setupConfig() {
  config = store$1.store;
  ipcMain.on(IPC.SET_CONFIG, (_e, configData) => {
    store$1.store = JSON.parse(configData);
    config = store$1.store;
    unRegisterShortcut();
    registerShortcut();
    setCookie();
...
function registerShortcut() {
  globalShortcut.register(config.priceCheckHotkey, () => {
    getClipboard().then((clip) => togglePriceCheck(clip)).catch((err) => console.log(err));
...
function pasteTextToChat(text, lastMsg, moveToFront) {
...
const baseURL = `${"https://pathofexile.tw"}/api/`;
const GGCapi = axios.create({
  baseURL,
  timeout: 4e3,
...
function getStats$3(src2, dest, opts) {
...
function checkPaths(src2, dest, funcName, opts, cb) {
  util$1.callbackify(getStats$3)(src2, dest, opts, (err, stats) => {
    if (err)
      return cb(err);
...
function checkParentDir(destStat, src2, dest, opts, cb) {
  ...
  pathExists$5(destParent, (err, dirExists) => {
    if (err)
      return cb(err);
    if (dirExists)
      return getStats$2(destStat, src2, dest, opts, cb);
...
function startCopy$1(destStat, src2, dest, opts, cb) {
  if (opts.filter)
    return handleFilter(getStats$2, destStat, src2, dest, opts, cb);
  return getStats$2(destStat, src2, dest, opts, cb);
}
function getStats$2(destStat, src2, dest, opts, cb) {
...
(function(module, exports) {
  ...
  var argsTag = "[object Arguments]", arrayTag = "[object Array]", asyncTag = "[object AsyncFunction]", boolTag = "[object Boolean]", dateTag = "[object Date]", errorTag = "[object Error]", funcTag = "[object Function]", genTag = "[object GeneratorFunction]", mapTag = "[object Map]", numberTag = "[object Number]", nullTag2 = "[object Null]", objectTag = "[object Object]", promiseTag = "[object Promise]", proxyTag = "[object Proxy]", regexpTag = "[object RegExp]", setTag = "[object Set]", stringTag = "[object String]", symbolTag2 = "[object Symbol]", undefinedTag2 = "[object Undefined]", weakMapTag = "[object WeakMap]";
...
function getAppCacheDir() {
  ...
  if (process.platform === "win32") {
    ...
    result = path$2.join(homedir, "Library", "Application Support", "Caches");
  } else {
...
function setupItemEntries(itemArray, heistReward) {
  const itemBaseType = [];
  itemArray.slice().reverse().forEach((item) => {
    ...
    if (((_a = item.flags) == null ? void 0 : _a.unique) === true) {
      itemBaseType[index].unique.push({
        name: item.name,
        text: item.text
      });
      if ((_b = item.name) == null ? void 0 : _b.startsWith("贗品")) {
        heistReward.push({
          name: item.name,
          type: item.type,
          text: item.text
...
function parseGams(gemEntries) {
  ...
  gemEntries.forEach((gem) => {
    ...
    if (gem.disc && gem.disc.startsWith("alt_")) {
      ...
      if (sameTypeGem) {
        (_a = sameTypeGem.trans) == null ? void 0 : _a.push({
          text: gem.text,
          disc: gem.disc
        });
      } else {
  ...
  itemsJson.result.forEach((itemGroup) => {
    ...
    switch (groupID) {
      ...
      case "gems":
        APIitems[groupID] = {
          ...itemGroup,
          entries: parseGams(itemGroup.entries),
          id: "gems"
        };
        if (itemGroup.id === "gems")
...
function checkNewline(statsGroup) {
  const mutiLines = [];
  statsGroup.entries.forEach((stat2) => {
    if (stat2.text.includes("\n")) {
      const lines = stat2.text.split("\n");
      mutiLines.push({
        id: stat2.id,
        text: lines
      });
    ...
    switch (statsGroup.id) {
      case "pseudo":
        APImods.pseudo = {
          ...
          entries: statsGroup.entries.filter((stat2) => stat2.text.indexOf("有房間：") === -1).map((stat2) => ({
            id: stat2.id,
            text: stat2.text,
            option: stat2.option
          })),
        ...
        APImods.temple = {
          ...
          entries: statsGroup.entries.filter((stat2) => stat2.text.indexOf("有房間：") > -1).map((stat2) => ({
            id: stat2.id,
            text: stat2.text.substring(4).replace(/（階級 [123]）/, ""),
            value: {
              option: 1
            }
          })),
      ...
      case "explicit":
        ...
        APImods.explicit = {
          ...
          entries: statsGroup.entries.map((ele) => ({
            ...ele
          })).filter((stat2) => !stat2.text.includes("\n")),
          type: "隨機"
        };
      ...
      case "enchant":
        APImods.clusterJewel = {
          label: statsGroup.label,
          entries: statsGroup.entries.splice(statsGroup.entries.findIndex((ele) => ele.text === "附加的小型天賦給予：#"), 1)[0].option.options.map((option) => ({
...
function setupAPIStatic(data) {
  ...
  data.forEach((group) => {
    var _a;
    if ((_a = group.label) == null ? void 0 : _a.match(/^地圖(（|\s\()|命運卡/))
      return;
...
mainExports.autoUpdater.on("update-available", ({ version, releaseNotes }) => {
...
async function checkForUpdate() {
  var _a;
  if (await ((_a = session.defaultSession) == null ? void 0 : _a.getCacheSize()) >>> 20 >= 30) {
    session.defaultSession.clearCache().catch((err) => console.log(err));
    session.defaultSession.clearCodeCaches({}).catch((err) => console.log(err));
  }
...
function setupTray() {
  tray = new Tray(join$1(process.env.PUBLIC, "SextantOrb128.ico"));
  buildTray();
}
const require$1 = Module.createRequire(import.meta.url);
...
app.on("ready", async () => {
  ...
  try {
    ...
    const detailMessage = error instanceof Error ? (error == null ? void 0 : error.stack) ?? error.message : error;
    dialog.showMessageBox({
      title: "讀取API資料錯誤",
