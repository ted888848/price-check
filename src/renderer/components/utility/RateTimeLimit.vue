<template>
  <div class="bg-gray-800 text-white absolute bottom-0 left-0 transform -translate-x-full p-8px">
    <div>Rate Limit</div>
    <div>Search</div>
    <ul>
      <li v-for="(state, rule) in rateTimeLimitState.search" :key="rule" class="text-sm hover:cursor-default">
        {{ rule }}:
        <ul>
