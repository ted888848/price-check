<template>
	...
	<vSelect v-if="item.undefinedUnqiue && item.uniques !== []" class="text-sm style-chooser" :options="item.uniques"
		label="name" v-model="item.name" :reduce="ele => ele.name" />
	<div class="mx-0  bg-blue-900 grid grid-cols-3">
		<div class="flex p-2 items-center justify-center">
			...
			<vSelect class="text-sm style-chooser flex-grow" :options="generalOption" v-model="item.isCorrupt" label="label"
		...
		<div>
			<div class="flex p-2 items-center justify-center" v-if="!item.type.text.endsWith('技能寶石')">
				...
				<vSelect class="text-sm style-chooser flex-grow" :options="generalOption" v-model="item.isIdentify"
			...
			<div class="flex p-2 items-center justify-center" v-else>
				...
				<vSelect class="text-sm style-chooser flex-grow" :options="gemAltQOptions" v-model="item.altQType" label="label"
					:reduce="ele => ele.value" :clearable="false" :searchable="false" />
			</div>
		</div>
		<div class="flex items-center justify-center">
			<div v-if="item.gemLevel" class="flex p-2 items-center justify-center"
				...
				<input class="w-8 appearance-none rounded bg-gray-400 text-center mx-1 font-bold" type="number"
					v-model.number="item.gemLevel.min" :disabled="!item.gemLevel.search" @dblclick="item.gemLevel.min = ''">
				<input class="w-8 appearance-none rounded bg-gray-400 text-center font-bold" type="number"
					v-model.number="item.gemLevel.max" :disabled="!item.gemLevel.search" @dblclick="item.gemLevel.max = ''">
			</div>
			<div v-else-if="item.mapTier" class="flex p-2 items-center justify-center"
				...
				<input class="w-8 appearance-none rounded bg-gray-400 text-center mx-1 font-bold" type="number"
					v-model.number="item.mapTier.min" :disabled="!item.mapTier.search" @dblclick="item.mapTier.min = ''">
				<input class="w-8 appearance-none rounded bg-gray-400 text-center font-bold" type="number"
					v-model.number="item.mapTier.max" :disabled="!item.mapTier.search" @dblclick="item.mapTier.max = ''">
			</div>
		...
		<div class="flex p-2 items-center justify-center" :class="{ 'opacity-30': !item.quality.search }"
			...
			<input class="w-8 appearance-none rounded bg-gray-400 text-center mx-1 font-bold" type="number"
				v-model.number="item.quality.min" :disabled="!item.quality.search" @dblclick="item.quality.min = ''">
			<input class="w-8 appearance-none rounded bg-gray-400 text-center font-bold" type="number"
				v-model.number="item.quality.max" :disabled="!item.quality.search" @dblclick="item.quality.max = ''">
		</div>
		<div v-if="item.elderMap" class="flex col-span-2 items-center justify-center">
			...
			<vSelect class="text-sm style-chooser style-chooser-inf " :options="elderMapOptions"
				v-model="item.elderMap.value.option" :reduce="ele => ele.value" label="label" :searchable="false"
				:clearable="false" />
		</div>
		<div v-if="item.conquerorMap" class="flex col-span-2 items-center justify-center">
			...
			<vSelect class="text-sm style-chooser style-chooser-inf " :options="conquerorMapOptions"
				v-model="item.conquerorMap.value.option" :reduce="ele => ele.value" label="label" :searchable="false"
				:clearable="false" />
		</div>
		...
		<div v-else-if="item.isWeaponOrArmor || ['項鍊', '戒指', '腰帶', '箭袋'].includes(item.type.text)"
			...
			<div class=" flex-grow mx-1">
				<vSelect class="text-sm style-chooser style-chooser-inf " :options="influencesOptions" v-model="item.influences"
		...
		<div class="flex items-center justify-center py-1">
			...
			<vSelect class="text-sm style-chooser w-24" :options="rarityOptions" v-model="item.raritySearch" label="label"
	...
	<table v-if="searchStats.length" class="bg-gray-700 text-center mt-1 text-white text-sm">
		...
		<tbody v-show="modTbodyToggle" class="modsTbody" style="">
			<tr v-for="mod in searchStats" :key="mod.id" class=" border-b-2 border-gray-400">
				...
				<td @click="mod.disabled = !mod.disabled" class=" text-lg font-semibold hover:cursor-default"
					:style="{ color: modTextColor(mod.type) }">{{ mod.type }}</td>
...
const { generalOption, gemAltQOptions, influencesOptions, elderMapOptions,
	conquerorMapOptions, rarityOptions } = selectOptions
