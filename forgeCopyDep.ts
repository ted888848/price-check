...
async function lookupNodeModulesPaths(root: string, paths: string[] = []): Promise<string[]> {
...
export async function copyDeps(_forgeConfig: ForgeConfig, buildPath: string): Promise<void> {
  const pj = await fs.readJson(path.resolve('.', 'package.json'))
  const flatDependencies = await getFlatDependencies(path.resolve('.'))
